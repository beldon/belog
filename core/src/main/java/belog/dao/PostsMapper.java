...
public interface PostsMapper {
    ...
    @Select({
            ...
            "where post.id = relation.object_id and relation.type = #{type} and relation.taxonomy_id = #{taxId}",
            "order by post.cre_date desc"
    })
    @ResultMap("ResultMapWithBLOBs")
    List<Posts> findPageByTypeAndTaxId(@Param("taxId") Long id, @Param("type") String type, Page<Posts> page);
}