...
public abstract class FrontBaseController {
    ...
    protected Boolean isFileExist(String theme) {
    ...
    protected boolean ifFolderExist(String folder) {
        String themePath = "/template/theme/"
                + themeService.getThemeName()
                + "/" + folder;
        File file = new File(appContext.getWebAppRoot() + themePath);
        if (file.isDirectory() && file.exists()) {
            logger.info("尝试使用文件夹：" + themePath + "【存在】");
            return true;
        } else {
            logger.info("尝试使用文件夹：" + themePath + "【不存在】");
            return false;
        }
    }
